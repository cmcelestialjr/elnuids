$(document).ready(function() {
    ...
    $(document).off('change', '#select-pagination')
    .on('change', '#select-pagination', function (e) {
        paginate(1);
...
function paginate(page) {
    ...
    var select = $('#select-pagination option:selected').val();
    var form_data = {
        value:value,
        page:page,
        select:select
    ...
    $.ajax({
        url: base_url+'/dts/forwardedPaginate',
        type: 'GET',
        headers: {
        ...
        success : function(data){
            ...
            updateDisplayedContent(data,select);
        },
        error: function (){
...
function updateDisplayedContent(data,select) {
    ...
    if(data.list.length<=0){
        var row = '<tr>';
            if(select=='returned'){
                row += '<td colspan="11" class="center" style="padding-top:50px;padding-bottom:50px"><h4>No document returned yet...</h4></td>';
            }else{
                row += '<td colspan="11" class="center" style="padding-top:50px;padding-bottom:50px"><h4>No document forwarded yet...</h4></td>';
            }
            row += '</tr>';
