$(document).off('blur', 'input[name="lastname"]').on('blur', 'input[name="firstname"]', function (e) {
    checkName();
});
$(document).off('blur', 'input[name="firstname"]').on('blur', 'input[name="firstname"]', function (e) {
    checkName();
});
$(document).off('click', '.employee-btn').on('click', '.employee-btn', function (e) {
    var employee = $(this);
    $('#employee_id').val(employee.data('id'));
    $('#is_selected').val('yes');
    $('#lastname').val(employee.data('lastname'));
    $('#firstname').val(employee.data('firstname'));
    $('#middlename').val(employee.data('middlename'));
    $('#extname').val(employee.data('extname'));
    $('#dob').val(employee.data('dob'));
    $('#sex').val(employee.data('sex')).trigger('change');
    $('#civil_status').val(employee.data('civilstatus')).trigger('change');
    $('#email').val(employee.data('email'));
    $('#contact').val(employee.data('contact'));    
});
$(document).off('click', 'button[name="submit_new_employee"]').on('click', 'button[name="submit_new_employee"]', function (e) {
    ...
    var is_selected = $('input[name="is_selected"]').val();
    ...
    if(x==0){
        ...
        $.ajax({
            url: base_url+'/hrims/employee/employeeNewSubmit',
            type: 'POST',
            headers: {
...
function checkName(){
    ...
    if(lastname != '' && firstname != ''){
        ...
        $.ajax({
            url: base_url+'/hrims/employee/checkName',
            type: 'POST',
            headers: {
                'X-CSRF-TOKEN': CSRF_TOKEN
            },
                ...
                if(response.result=='success'){
                    ...
                    if (response.employees.length > 0) {
                        ...
                        $.each(employees, function(index, employee) {
                            let button = `
                                <button class="btn btn-info btn-info-scan employee-btn" 
                                        data-id="${employee.id}" 
                                        data-lastname="${employee.lastname}" 
                                        data-firstname="${employee.firstname}" 
                                        data-middlename="${employee.middlename}" 
                                        data-extname="${employee.extname}" 
                                        data-dob="${employee.dob}" 
                                        data-sex="${employee.sex}" 
                                        data-civilstatus="${employee.civil_status}" 
                                        data-email="${employee.email}" 
                                        data-contact="${employee.contact}">
                                    ${employee.lastname}, ${employee.firstname} ${employee.extname} ${employee.middlename} 
                                </button>
                            `;
                            $('#employee-buttons').append(button);
                        });
                    }
                }else{
