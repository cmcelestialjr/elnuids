...
$(document).off('click', '#dtrDiv .view_option_btn').on('click', '#dtrDiv .view_option_btn', function (e) {
    var view_option = $(this).val();
    if(view_option=='dtr_view'){
        dtr_table();
    }else{
        time_logs_table();
    }
...
$(document).off('click', '.timeLogsPrint').on('click', '.timeLogsPrint', function (e) {
    var thisBtn = $(this);
    timeLogsPrint(thisBtn);
});
...
function time_logs_table(){
    ...
    $.ajax({
        url: base_url+'/hrims/dtr/individual_time_logs',
        type: 'POST',
        headers: {
            'X-CSRF-TOKEN': CSRF_TOKEN
        },
...
function timeLogsPrint(thisBtn){
    var id_no = $('input[name="id_no"]').val();
    var option = thisBtn.data('id');
    var year = thisBtn.data('y');
    var month = thisBtn.data('m');
    var c = thisBtn.data('c');
    window.open(base_url+'/hrims/timeLogs/pdf/'+c+'/'+id_no+'/'+option+'/'+year+'/'+month, '_blank');
}
