...
$(document).off('blur', '#deductionModalDiv .others_input').on('blur', '#deductionModalDiv .others_input', function (e) {
    var thisBtn = $(this);
    othersInput(thisBtn);
});
$(document).off('click', '#deductionModalDiv #allowanceLi').on('click', '#deductionModalDiv #allowanceLi', function (e) {
...
$(document).off('click', '#deductionModalDiv #zeroEarned').on('click', '#deductionModalDiv #zeroEarned', function (e) {
    var thisBtn = $(this);
    zeroEarned(thisBtn);
});
...
$(document).off('click', '#deductionModalDiv #lwopModal').on('click', '#deductionModalDiv #lwopModal', function (e) {
    var thisBtn = $(this);
    lwopModal(thisBtn);
});
function list_table(){
    var url_option = $('input[name="url_option"]').val();
    var form_data = {
...
function lwopModal(thisBtn){

    const lwop_value = parseFloat($('#deductionModalDiv input[name="lwop_value"]').val());

    Swal.fire({
        title: 'Update LWOP',
        // text: 'Do you want to zero the earned amount of this employee?',
        // icon: 'info',
        input: 'number',
        inputLabel: 'LWOP',
        inputPlaceholder: 'Enter LWOP value',
        inputValue: lwop_value,
        ...
        if (result.isConfirmed) {
            ...
            if (lwop) {
                ...
                var url = base_url + '/hrims/payroll/view/lwopModalSubmit';
                $.ajax({
                    url: url,
                    type: 'POST',
                    headers: {
                    ...
                    success: function(response) {
                        if(response.result=='success'){
                            ...
                            $('#deductionModalDiv input[name="lwop_value"]').val(lwop);
                            $('#deductionModalDiv #lwopModal').html(lwop);
                            updateValues(id,response);
                        }else{
                            Swal.fire({
                                title: 'Error.',
                ...
                Swal.fire({
                    title: 'Oops!',
                    text: 'Please enter a reason before proceeding.',
                    icon: 'warning'
                });
            }
        }
...
function zeroEarned(thisBtn){
    Swal.fire({
        title: 'Are you sure?',
        text: 'Do you want to Zero the earned of this employee?',
        icon: 'warning',
        showCancelButton: true,
        confirmButtonColor: '#3085d6',
        cancelButtonColor: '#d33',
        ...
        if (result.isConfirmed) {
            var id = $('#deductionModalDiv input[name="id"]').val();
            var isChecked = thisBtn.is(':checked');
            var zero = 'no';
            if (isChecked) {
                var zero = 'yes';
            }
            ...
            $.ajax({
                url: base_url+'/hrims/payroll/view/deductionModalZero',
                type: 'POST',
                headers: {
                    'X-CSRF-TOKEN': CSRF_TOKEN
                },
...
function othersInput(thisBtn){
    ...
    $.ajax({
        url: base_url+'/hrims/payroll/view/othersInput',
        type: 'POST',
        headers: {
            'X-CSRF-TOKEN': CSRF_TOKEN
        },
